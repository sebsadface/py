...
class NaiveBayes():
    ...
    def predict(self, filename:str):
        ...
        p_ham:float = 0.0
        p_spam:float = 0.0
        total_email:int = self.num_train_hams + self.num_train_spams
        for word in self.word_set(filename):
            p_ham += np.log((self.word_counts_ham.get(word, 0) + 1) / (self.num_train_hams + 2))
            p_spam += np.log((self.word_counts_spam.get(word, 0) + 1) / (self.num_train_spams + 2))
         
        if (np.log(self.num_train_spams / total_email) + p_spam) > (np.log(self.num_train_hams / total_email) + p_ham):
