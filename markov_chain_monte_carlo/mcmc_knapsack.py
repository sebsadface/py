...
class KnapsackMcmc:
    ...
    def mcmc(self, W:float, T:float=0, num_iter:int=5000):
        ...
        best_subset = np.copy(subset)
        l = []
        for t in range(num_iter):
            l.append(self.value(subset))
            k = np.random.randint(0,len(subset)) 
            new_subset = np.copy(subset)
            if new_subset[k] == 1:
                new_subset[k]= 0
            else:
                new_subset[k]= 1
            delta = self.value(new_subset) - self.value(subset)
            if self.weight(new_subset) <= W:
                if (delta > 0) or (T> 0 and np.random.rand() < np.exp(delta/T)):
                    subset = np.copy(new_subset)
            if self.value(subset) > self.value(best_subset):
                best_subset = np.copy(subset)
        print(T)
        print(W)
        print(num_iter)
        return best_subset, self.value(best_subset), l
